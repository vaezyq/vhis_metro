...
import com.missiongroup.starring.core.vehicleData.vehicleBean.CBBean;
...
public class CBDataFactory extends DatatFactory {
    static List<CBConfig> faultConfigs = iCaches.get("CONSTANT", "cbConfigs");
    ...
    public static Map createData() {
        ...
        Map C1 = createCBC1();
        Map C2 = createCBC2();
        data.put("A1", buildCBData(A1,"TC1"));
        data.put("A2", buildCBData(A2,"TC2"));
        data.put("B1", buildCBData(B1,"MP1"));
        data.put("B2", buildCBData(B2,"MP2"));
        data.put("C1", buildCBData(C1,"M2"));
        data.put("C2", buildCBData(C2,"M3"));
        return data;
    }

    private static List buildCBData(Map data,String t) {
        ArrayList<CBBean> cbBeans = new ArrayList<>();
        for (CBConfig faultConfig : faultConfigs) {
            String car = faultConfig.getCar();
            String name = faultConfig.getName().replaceAll("\\s*", "");
            String status = String.valueOf(data.get(name));
            if(!status.equals("null")&&car.equals(t)){
                CBBean cbBean = new CBBean();
                cbBean.setDescription(faultConfig.getDescription());
                cbBean.setName(faultConfig.getName().trim());
                cbBean.setStatus(status);
                cbBeans.add(cbBean);
            }
    ...
    private static Map createCBC1() {
        Map<String, Object> data = new HashMap<>();
        Long LI_HVACCB_C1 = getValue("port0x3b4", "LI_HVACCB_C1");
        data.put("HVACCB", LI_HVACCB_C1 == 1 ? "true" : "false");
    ...
    private static Map createCBC2() {
        Map<String, Object> data = new HashMap<>();
        Long LI_HVACCB_C2 = getValue("port0x3d4", "LI_HVACCB_C2");
        data.put("HVACCB", LI_HVACCB_C2 == 1 ? "true" : "false");
    ...
    private static Map createCBB2() {
        Map<String, Object> data = new HashMap<>();
        Long LI_HVACCB_B2 = getValue("port0x3f4", "LI_HVACCB_B2");
        data.put("HVACCB", LI_HVACCB_B2 == 1 ? "true" : "false");
    ...
    private static Map createCBA2() {
        Map<String, Object> data = new HashMap<>();
        Long LI_PANTCB_A2 = getValue("port0x434", "LI_PANTCB_A2");
        data.put("PANTCB", LI_PANTCB_A2 == 1 ? "true" : "false");
